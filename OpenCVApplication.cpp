...
void initPieces()
{
	...
	for (int i = 0; i < table.rows; i++)
		for (int j = 0; j < table.cols * 3; j+=3)
...
void processing()
{
	Mat aux = imread("Images/test.png", CV_LOAD_IMAGE_COLOR);
	imshow("test1", aux);
	//Mat aux = table.clone();
	for (int i = 0; i < table.rows; i++)
	{
		for (int j = 0; j < table.cols * 3 ; j +=3 * 1)
		{
			//aux.at<uchar>(i, j) = table.at<uchar>(i, j);
			/*
			for (std::vector<Piece>::iterator it = unique.begin(); it != unique.end(); ++it)
			{
				...
				int ii = 0;
				int jj = 0;
				for ( ii = 0; ii < src.rows && ok; ii++)
					for ( jj = 0; jj < src.cols  && ok; jj++)
						if (table.at<uchar>(i + ii, j + jj) != src.at<uchar>(ii, jj))
							ok = false;

				if (ok)
				{
					printf("%s\t", piesa.getName().c_str());
					i += ii;
					j += jj;
				}
					//printf("da");
	...
	for (int i = 0; i < table.rows; i++)
	{
		for (int j = 0; j < table.cols * 3; j += 3 * 1)
		{
			...
			for (std::vector<Piece>::iterator it = unique.begin(); it != unique.end(); ++it)
			{
				...
				int jj = 0;
				for (ii = 0; ii < src.rows && ok; ii++)
					for (jj = 0; jj < src.cols * 3 && ok; jj += 3)
						if (ii + i < table.rows && jj + j < table.cols *3 )
						{
							if (table.at<uchar>(i + ii, j + jj) != src.at<uchar>(ii, jj))
								ok = false;
						}
